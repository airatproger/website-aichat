...
(() => {
  ...
  function createRuntime(doc = document) {
    ...
    function ensureFetched(name) {
      ...
      if (r.fetched) return;
      r.fetched = true;
      const filename = encodeURIComponent(name) + ".dc.html";
      const resources = typeof window !== "undefined" && window.__resources;
      const url = resources && resources[filename] || COMPONENT_DIR + "/" + filename;
      fetch(url).then((res) => {
        if (!res.ok) {
          console.error(
